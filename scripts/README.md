...
| Argument| Description                | Example                      |
|---------|----------------------------|------------------------------|
| -c      | Path to configuration file | ../cfg/drone_to_las.cfg |
| -p      | Path to application PID    | /tmp/drone_to_las       |
| others  | -                          | -                            |
...
- [rename.sh](rename.sh)
    ...
    Renames all lines with drone_to_las in the whole project to the new project naming according to the script input argument.

*Example:* <br>

```./scripts/rename.sh drone_to_las new_service preview``` - preview change files and lines.

```./scripts/rename.sh drone_to_las new_service``` - recursively replace first argument to second in all files.

| Argument      | Description                        | Example            |
|---------------|------------------------------------|--------------------|
| №1            | The name to be replaced            | drone_to_las  |
